...
def get_secrets_file(environment):
    """Get the secrets file path for database configuration."""
    project_root = get_project_root()
    secrets_file = project_root / ".envs" / f".{environment}" / ".secrets"
    return secrets_file if secrets_file.exists() else None
...
def get_database_config(environment):
    """Get database configuration from both .django and .secrets files.
    Secrets file values override .django file values.
    """
    django_file = get_env_file(environment)
    secrets_file = get_secrets_file(environment)

    # Start with .django file
    config = parse_env_file(django_file)

    # Override with .secrets file if it exists
    if secrets_file:
        secrets_config = parse_env_file(secrets_file)
...
def backup_database(environment, output_file=None, format_type='sql'):
    """Create a database backup.

    Args:
        environment: Environment name (local, production)
        output_file: Optional output filename
        format_type: Backup format - 'sql' (plain text, default) or 'binary' (custom format)
    """
    ...
    # Determine file extension and format flag
    if format_type == 'binary':
        extension = '.dump'
        format_flag = '-Fc'  # Custom format (binary)
        file_mode = 'wb'
        encoding = None
    else:  # Default to SQL format
        extension = '.sql'
        format_flag = '-Fp'  # Plain text format
        file_mode = 'w'
        encoding = 'utf-8'
    ...
    if output_file:
        if os.path.isabs(output_file):
            ...
            # If no extension provided, add appropriate extension
            if not output_file.endswith(('.sql', '.dump')):
                backup_path = backup_dir / f"{output_file}{extension}"
            else:
                backup_path = backup_dir / output_file
    else:
        timestamp = datetime.now().strftime("%Y_%m_%dT%H_%M_%S")
        backup_path = backup_dir / f"{environment}_db_{timestamp}{extension}"
    ...
    cmd = [
        ...
        "exec", "-T", "postgres",
        "pg_dump", "-U", db_user, format_flag, "--no-owner", "--no-acl", db_name
    ]

    try:
        if format_type == 'binary':
            # Binary format: binary mode
            with open(backup_path, 'wb') as f:
                result = subprocess.run(cmd, stdout=f, stderr=subprocess.PIPE, check=True)
        else:
            # SQL format: text mode (default)
...
def restore_via_stdin(compose_file, backup_path, db_user, db_name):
    """Restore database by piping backup directly to pg_restore or psql."""
    is_sql = backup_path.suffix == '.sql'

    if is_sql:
        print("Restoring database via stdin using psql...")
        cmd = [
            "docker", "compose", "-f", str(compose_file),
            "exec", "-T", "postgres",
        ...
        print("Restoring database via stdin using pg_restore...")
        cmd = [
            "docker", "compose", "-f", str(compose_file),
            "exec", "-T", "postgres",
...
def list_backup_files():
    ...
    backup_dir = get_backup_dir()
    backup_files = sorted(
        [f for f in backup_dir.iterdir() if f.is_file() and f.suffix in ('.dump', '.sql')],
...
def restore_database(environment, backup_file, target_db=None):
    ...
    # Determine file extension for container path
    is_sql = backup_path.suffix == '.sql'
    container_file = "/tmp/restore.dump" if not is_sql else "/tmp/restore.sql"
    ...
    try:
        ...
        subprocess.run(
            ["docker", "compose", "-f", str(compose_file), "cp", str(backup_path), f"postgres:{container_file}"],
            check=True,
            capture_output=True
        )
        ...
        # Determine file extension for container path
        container_file = "/tmp/restore.dump" if backup_path.suffix == '.dump' else "/tmp/restore.sql"

        print(f"Copying backup file to container '{container_name}'...")
        try:
            subprocess.run(
                ["docker", "cp", str(backup_path), f"{container_name}:{container_file}"],
                check=True
            )
        except subprocess.CalledProcessError as e:
            print(f"Error copying backup to container: {e}", file=sys.stderr)
            print("Trying alternative method: using stdin for restore...", file=sys.stderr)
            restore_via_stdin(compose_file, backup_path, db_user, db_name)
            return

    # Restore database - use psql for SQL files, pg_restore for custom format
    is_sql = backup_path.suffix == '.sql'
    container_file = "/tmp/restore.dump" if not is_sql else "/tmp/restore.sql"
    ...
    if is_sql:
        ...
        cmd = [
            "docker", "compose", "-f", str(compose_file),
            "exec", "-T", "postgres",
            "psql", "-U", db_user, "-d", db_name, "-f", container_file
        ]
    else:
        ...
        cmd = [
            "docker", "compose", "-f", str(compose_file),
            "exec", "-T", "postgres",
            "pg_restore", "-U", db_user, "-d", db_name, "--no-owner", "--no-acl", container_file
        ]
    ...
    try:
        ...
        try:
            subprocess.run(
                ["docker", "compose", "-f", str(compose_file),
                 "exec", "-T", "postgres", "rm", container_file],
                check=True,
                capture_output=True
            )
        except subprocess.CalledProcessError:
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Create a backup from local environment (SQL format, default)
  python scripts/db_backup.py backup local

  # Create a backup in binary format (smaller, faster)
  python scripts/db_backup.py backup local --format binary

  # Create a backup with custom filename (SQL format by default)
  python scripts/db_backup.py backup local my_backup.sql

  # Create a binary backup with custom filename
  python scripts/db_backup.py backup local my_backup.dump --format binary

  # Restore from backup (interactive selection if backup_file not provided)
  python scripts/db_backup.py restore local

  # Restore from specific backup file
  python scripts/db_backup.py restore local my_backup.sql

  # Restore from binary backup file
  python scripts/db_backup.py restore local my_backup.dump

  # Restore to a different database
  python scripts/db_backup.py restore local my_backup.sql other_db_name
        """
    )
    ...
    backup_parser.add_argument('--format', choices=['sql', 'binary'], default='sql',
                             help='Backup format: sql (plain text, default) or binary (custom format). Default: sql')
    ...
    if args.command == 'backup':
        backup_database(args.environment, args.output_file, args.format)
