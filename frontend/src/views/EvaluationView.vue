...
import type { EssayRequest, EssayEvaluationDataBrief } from '@/types/essay'
...
// Extend EssayEvaluationDataBrief type to include essay_prompt
interface ExtendedEssayEvaluationDataBrief extends EssayEvaluationDataBrief {
  essay_prompt?: string;
}
...
const userLevel = ref('Advanced Writer')
...
const selectedOptions = ref<string[]>(['structure', 'arguments'])
...
const wordCount = computed(() => form.values.essay_text?.split(/\s+/).filter(word => word.length > 0).length || 0)
...
const canAnalyze = computed(() => {
  return form.values.essay_prompt?.trim() &&
